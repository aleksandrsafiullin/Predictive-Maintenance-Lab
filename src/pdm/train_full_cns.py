...
def main(argv=None):
    ...
    mcfg["reservoir"]["n_nodes"] = model.n_nodes
    print(f"Computing neurons: {model.n_nodes:,}; directed pairs: {model.provenance['n_edges']:,}; "
          f"synapses: {model.provenance['n_synapses']:,}; readout groups: {model.n_readout_features}", flush=True)
    _check_stop()
    base = {"version": 1, "warmup_measurements": 20, "smoothing_tau_s": DEFAULT_TAU_S, "n_nodes": model.n_readout_features}
